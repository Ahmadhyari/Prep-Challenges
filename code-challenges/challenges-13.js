...
const objectCounter = (obj) => {
    return obj.reduce((result, value) => ++result, 0);
}
...
const statistics = (obj) => {
    let result = obj.reduce((value, element) => {
        if (value[element.votes_To])
        value[element.votes_To]++;
        else value[element.votes_To] = 1;
