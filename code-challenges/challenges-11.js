...
function gradesAvg(arr) {
        ...
        let avgResult = arr.map((value => ({ ...value, avg: (value.gradsList.reduce((x, y) => (x + y))) / value.gradsList.length })));
...
function studentsResult(arr) {
    const result = arr.map((value => ({
        ...value, ...(((value.gradsList.reduce((x1, y1) => x1 + y1)) / value.gradsList.length) >= 50 && { result: "Passed" }),
        ...(((value.gradsList.reduce((x1, y1) => x1 + y1)) / value.gradsList.length) < 50 && { result: "Failed" }),
    })))
    return result;
